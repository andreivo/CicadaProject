...
FileSD SDClass::open(const char *filepath, uint8_t mode) {
    /*
    ...
    return FileSD(file, filepath);
}


/*
FileSD SDClass::open(char *filepath, uint8_t mode) {
  //
...
FileSD FileSD::openNextFile(uint8_t mode) {
    dir_t p;
...
void FileSD::rewindDirectory(void) {
    if (isDirectory())
        _file->rewind();
}
