...
#include "../System/SPIFFS/CicadaSPIFFSFile.h"
...
//Station
const char HTTP_FORM_CONFIG_STATION[] PROGMEM = "<form name='frmConfig' method='post' action='/saveCicadaStation' onsubmit='return validate()'>{cicadalogo}<h1>Cicada DCP Wizard</h1><h2>1-4: Station Configuration</h2><p id='msg' style='color:red'></p><h4>Owner</h4><p>Name</p><input name='owname' value='{owname}'><p>Valid e-mail</p><input name='owemail' value='{owemail}'><p>Phone</p><input name='owphone' value='{owphone}'><br><h4>Station</h4><p>Station Name</p><input name='name' value='{name}' maxlength='15'><p>Station Password</p><input name='spass' value='{spass}'><p>Latitude</p><input name='lat' value='{lat}'><p>Longitute</p><input name='lon' value='{lon}'><p>Time slot to send (minutes)</p><input name='sti' value='{sti}'><p>Time slot store metadata (min)</p><input name='smi' value='{smi}'><br><h4>Self update</h4><p>Host</p><input name='suhost' value='{suhost}'><p>Path in the host</p><input name='supath' value='{supath}'><p>Port</p><input name='suport' value='{suport}'><p>Daily time to check for updates (hour)</p><input name='sudtupdate' value='{sudtupdate}'><br><input type='hidden' name='ttr'> <button type='submit'>Save and Next</button></form><br><div class='c'><a href='/sensors'>Sensors Configuration</a></div>";
const char HTTP_SCRIPT_FORM_CONFIG_STATION[] PROGMEM = "<script type='text/javascript'>function validate(){msg.innerHTML='';var e=!0,a=document.forms.frmConfig,n=(parseFloat(a.vol.value),parseFloat(a.sti.value)),r=parseFloat(a.smi.value),t=parseFloat(a.lat.value),s=parseFloat(a.lon.value),i=(a.ttr,a.name.value),u=a.spass.value,m=a.suhost.value,l=a.supath.value,o=parseInt(a.suport.value),g=parseInt(a.sudtupdate.value),d=a.owname.value,v=a.owemail.value;return!d&&(msg.innerHTML+='- Owner name is required!<br>',e=!1),!v&&(msg.innerHTML+='- Owner e-mail is required!<br>',e=!1),i&&/^([a-z0-9\-\_]+)$/gi.test(i)||(msg.innerHTML+='- Invalid name. Must contain only (aA-zZ), (0-9), (-) or (_)<br>',e=!1),(!u||u.length<8||u.length>64)&&(msg.innerHTML+='- The password must have between 8 and 64 char.<br>',e=!1),(!t||isNaN(t))&&(msg.innerHTML+='- Latitude is required!<br>',e=!1),(!s||isNaN(s))&&(msg.innerHTML+='- Longitude is required!<br>',e=!1),(!n||isNaN(n)||5>n||n>30)&&(msg.innerHTML+='- The time slot to send must be between 5 and 30 min (e.g.: 10)<br>',e=!1),(!r||isNaN(r)||5>r||r>60)&&(msg.innerHTML+='- The time slot store metadata must be between 5 and 60 min (e.g.: 10)<br>',e=!1),m&&!l&&(msg.innerHTML+='- Invalid Path in the host.<br>',e=!1),m&&(!o||isNaN(o)||0>o)&&(msg.innerHTML+='- Invalid port number.<br>',e=!1),m&&(!g||isNaN(g)||0>=g||g>23)&&(msg.innerHTML+='- Daily time to check for updates must be between 0 and 23 hours (e.g.: 10)<br>',e=!1),e}var msg=document.getElementById('msg');</script>";

//Sensors
const char HTTP_FORM_CONFIG_SENSORS[] PROGMEM = "<form name='frmSensors' method='post' action='/saveSensorsConfig' onsubmit='return validate()'>{cicadalogo}<h1>Cicada DCP Wizard</h1><h2>1.1-4: Sensors Configuration</h2><p id='msg' style='color:red'></p><br><h4>DHT</h4><p>Time slot to collection (minutes)</p><input name='collDHT' value='{collDHT}'><br><h4>DHT - Temperature</h4><p>Code</p><input name='codetemp' value='{codetemp}'><p>DataType</p><input name='dttemp' value='{dttemp}'><br><h4>DHT - Humidity</h4><p>Code</p><input name='codehum' value='{codehum}'><p>DataType</p><input name='dthum' value='{dthum}'><br><h4>Pluviometer</h4><p>Code</p><input name='codeplu' value='{codeplu}'><p>DataType</p><input name='dtplu' value='{dtplu}'><p>Pluviometer Bucket Calibration</p><input name='pluvol' value='{pluvol}'><p>Pluviometer collection area (mm²)</p><input name='pluarea' value='{pluarea}'><p>Time slot to collection (minutes)</p><input name='collplu' value='{collplu}'><br><h4>Input Vcc</h4><p>Code</p><input name='codevin' value='{codevin}'><p>DataType</p><input name='dtvin' value='{dtvin}'><p>Time slot to collection (minutes)</p><input name='collvin' value='{collvin}'><br><h4>Battery Vcc</h4><p>Code</p><input name='codevba' value='{codevba}'><p>DataType</p><input name='dtvba' value='{dtvba}'><p>Time slot to collection (minutes)</p><input name='collvba' value='{collvba}'><br><button type='submit'>Save and Next</button></form>";
const char HTTP_SCRIPT_FORM_CONFIG_SENSORS[] PROGMEM = "<script type='text/javascript'>function validate(){msg.innerHTML='';var e=!0,a=document.forms.frmSensors,n=parseInt(a.codetemp.value),r=a.dttemp.value,t=parseInt(a.collDHT.value),i=parseInt(a.codehum.value),s=a.dthum.value,u=parseInt(a.codeplu.value),l=a.dtplu.value,m=parseInt(a.collplu.value),o=parseInt(a.codevin.value),v=parseFloat(a.pluvol.value),d=parseFloat(a.pluarea.value),b=a.dtvin.value,g=parseInt(a.collvin.value),T=parseInt(a.codevba.value),p=a.dtvba.value,c=parseInt(a.collvba.value);return(!n||isNaN(n))&&(msg.innerHTML+='- Code Temp. is required or inválid!<br>',e=!1),r||(msg.innerHTML+='- DataType Temp. is required!<br>',e=!1),(!t||isNaN(t)||5>t||t>60)&&(msg.innerHTML+='- The DHT collection time interval value must be between 5 and 60 min (e.g.: 10)<br>',e=!1),(!i||isNaN(i))&&(msg.innerHTML+='- Code Hum. is required or inválid!<br>',e=!1),s||(msg.innerHTML+='- DataType Hum. is required!<br>',e=!1),(!u||isNaN(u))&&(msg.innerHTML+='- Code Plu. is required or inválid!<br>',e=!1),l||(msg.innerHTML+='- DataType Plu. is required!<br>',e=!1),(!v||isNaN(v)||2>v||v>5)&&(msg.innerHTML+='- The bucket calibration value must be between 2.00 and 5.00 (e.g.: 2.47)<br>',e=!1),(!d||isNaN(d)||d>10)&&(msg.innerHTML+='- The pluviometer collection area value must be greater than 10mm²<br>',e=!1),(!m||isNaN(m)||5>m||m>60)&&(msg.innerHTML+='- The Pluv. collection time interval value must be between 5 and 60 min (e.g.: 10)<br>',e=!1),(!o||isNaN(o))&&(msg.innerHTML+='- Code Input Vcc is required or inválid!<br>',e=!1),b||(msg.innerHTML+='- DataType Input Vcc is required!<br>',e=!1),(!g||isNaN(g)||5>g||g>60)&&(msg.innerHTML+='- The Input Vcc collection time interval value must be between 5 and 60 min (e.g.: 10)<br>',e=!1),(!T||isNaN(T))&&(msg.innerHTML+='- Code Battery Vcc is required or inválid!<br>',e=!1),p||(msg.innerHTML+='- DataType Battery Vcc is required!<br>',e=!1),(!c||isNaN(c)||5>c||c>60)&&(msg.innerHTML+='- The Battery Vcc collection time interval value must be between 5 and 60 min (e.g.: 10)<br>',e=!1),e}var msg=document.getElementById('msg');</script>";
