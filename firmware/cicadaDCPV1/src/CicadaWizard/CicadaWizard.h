...
//Station
const char HTTP_FORM_CONFIG_STATION[] PROGMEM = "<form name='frmConfig' method='post' action='/saveCicadaStation' onsubmit='return validate();'>{cicadalogo}<h1>Cicada DCP Wizard</h1><h2>1-4: Station Configuration</h2><p id='msg' style='color: red'></p><p>Station Name</p><input type='text' name='name' value='{name}' maxlength='15'/><p>Station Password</p><input type='text' name='spass' value='{spass}'/><p>Latitude</p><input type='text' name='lat' value='{lat}' /><p>Longitute</p><input type='text' name='lon' value='{lon}' /><p>Bucket Calibration</p><input type='text' name='vol' value='{vol}' /><p>Send time interval (minutes)</p><input type='text' name='sti' value='{sti}' /><br><input type='hidden' name='ttr' /><button type='submit'>Save and Next</button></form><br/><div class='c'><a href='/sensors'>Sensors Configuration</a></div>";
const char HTTP_SCRIPT_FORM_CONFIG_STATION[] PROGMEM = "<script type='text/javascript'>var msg=document.getElementById('msg');function validate(){msg.innerHTML='';var result=true;var f=document.forms['frmConfig'];var vol=parseFloat(f['vol'].value);var sti=parseFloat(f['sti'].value);var lat=parseFloat(f['lat'].value);var lon=parseFloat(f['lon'].value);var ttr=f['ttr'];var name=f['name'].value;var pass=f['spass'].value;if(!lat || isNaN(lat)){msg.innerHTML +='- Latitude is required!<br>';result=false;}if(!lon || isNaN(lon)){msg.innerHTML +='- Longitude is required!<br>';result=false;}if(!vol || isNaN(vol) || (vol < 2.00 || vol > 5.00)){msg.innerHTML +='- The calibration value must be between 2.00 and 5.00 (e.g.: 2.47)<br>';result=false;}if(!sti || isNaN(sti) || (sti < 5.00 || sti > 30.00)){msg.innerHTML +='- The send time interval value must be between 5 and 30 min (e.g.: 10)<br>';result=false;}if(!name || !(/^([a-z0-9\-\_]+)$/ig.test(name))){msg.innerHTML +=' - Invalid name. Must contain only (aA-zZ), (0-9), (-) or (_)';result=false;}if(!pass){msg.innerHTML +=' - Password is required!';result=false;}var now=new Date();var nextReset=new Date();nextReset.setHours({RESET_TIME}, 0);if(now > nextReset){nextReset.setDate(nextReset.getDate() + 1);}var nowSecs=parseInt(now.getTime() / 1000);var nRs=parseInt(nextReset.getTime() / 1000);var ttrInSecs=parseInt(nRs - nowSecs);ttr.value=ttrInSecs;return result;}</script>";

//Sensors
const char HTTP_FORM_CONFIG_SENSORS[] PROGMEM = "<form name='frmSensors' method='post' action='/saveSensorsConfig' onsubmit='return validate()'>{cicadalogo}<h1>Cicada DCP Wizard</h1><h2>1.1-4: Sensors Configuration</h2><p id='msg' style='color:red'></p><br><h4>Temperature</h4><p>Code</p><input name='codetemp' value='{codetemp}'><p>DataType</p><input name='dttemp' value='{dttemp}'><p>Collection time interval (minutes)</p><input name='colltemp' value='{colltemp}'><br><h4>Humidity</h4><p>Code</p><input name='codehum' value='{codehum}'><p>DataType</p><input name='dthum' value='{dthum}'><p>Collection time interval (minutes)</p><input name='collhum' value='{collhum}'><br><h4>Pluviometer</h4><p>Code</p><input name='codeplu' value='{codeplu}'><p>DataType</p><input name='dtplu' value='{dtplu}'><p>Collection time interval (minutes)</p><input name='collplu' value='{collplu}'><br><h4>Battery Voltage</h4><p>Code</p><input name='codebtv' value='{codebtv}'><p>DataType</p><input name='dtbtv' value='{dtbtv}'><p>Collection time interval (minutes)</p><input name='collbtv' value='{collbtv}'><br><h4>Battery Current</h4><p>Code</p><input name='codebtc' value='{codebtc}'><p>DataType</p><input name='dtbtc' value='{dtbtc}'><p>Collection time interval (minutes)</p><input name='collbtc' value='{collbtc}'><br><button type='submit'>Save and Next</button></form>";
